...
und $p(x)-y \in \U_m$ gilt.
 %denn aufgrund der positiven Definitheit von Normen, gilt Gleichheit genau dann,
...
\]
Wenden wir uns nun nochmal dem Rayleigh-Ritz-Verfahren zu. Angenommen, uns st"unde
mit dem Suchraum $\U_m$ bereits ein von Eigenvektoren
aufgespannter Unterraum zur Verf"ugung ( die alle B orthonormal sind, X muss invertierbar sein oder reicht auch pseudoinverse? ). Dann w"are
$\Bild{P}$ nach Konstruktion wegen
\[
B^{-1}(A U_m) U_m^H B = B^{-1}(BU_m\Lambda) U_m^H B
= U_m\Lambda U_m^H B = U_m \Lambda U_m^{-1} = B^{-1}A
\]
ein $(B^{-1}A)$-invarianter Unterraum.
Daher erscheint es
wegen Satz \ref{thm:invariant} sinnvoll, den Spektralprojektor $P$ in den das Rayleigh-Ritz-Verfahren zu integrieren.
Da jedoch im Allgemeinen nicht bekannt ist, wie
$U_m$ aussehen muss, stellt sich die Frage, ob es
...
vorgegeben sind, die das abgeschlossene Intervall $I:=[\lambda_1, \lambda_2] \subseteq \R\subseteq\C$ definieren.
...
die um das vorgegebene Intervall $I$ heruml"auft, in der Gestalt \textcolor{red}{ACHTUNG HIER $I\subseteq\C$}
\[
...
Vektoren $x_3 \in\spn_\C\{e_3\}$ mit zugeh"origem Eigenwert $\lambda_3 = 0$
