...
\begin{defn}[Einfache geschlossene Kurve]\label{defn:chap3:jordanKurve}
...
\subcaption{Einfache geschl. Kurve.}
\end{subfigure}
\qquad
\begin{subfigure}[c]{.3\textwidth}
...
Wir k"onnen nun den Gedanken aus ~\cite[Abschnitt 4.9]{liesen} weiter folgen und betrachten f"ur $t\subseteq\Cn$ die durch
\[
...
\[
F(A_1,t) := \frac{1}{k}\sum_{j=1}^\infty(t^{-1}A_1)^j
\]
...
\end{equation}
Nach Konstruktion sind alle Eigenwerte von $A_1$ beziehungsweise die Singularit"aten von $(t I_m - A_1)^{-1}$ im Inneren von $\Gamma$. Der Wert des Integrals auf der linken Seite von Gleichung \eqref{eq:chap3:identitaet} ver"andert sich nicht, wenn wir anstelle von $\Gamma$ "uber eine
...
l"osen l"asst.\footnote{Hier bezeichnen $e_1, e_2, e_3 \in\C^3$ die Einheitsvektoren, also $e_1 = [1,0,0], e_2 = [0,1,0]$ und $e_3 = [0,0,1]$.}\\
...
vollen Rang und das $\Bild(P)$ ist invariant unter $B^{-1}A$.
