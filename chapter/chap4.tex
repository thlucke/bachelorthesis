...
Nachdem wir dieser kurzen Zusammenfassung,
widmen wir uns nun der Approximation des Spektralprojektors. Dazu nehmen wir uns das HPS-Eigenwertproblem $(A,B)$ und ein reelles Intervall $I := [\lambda_1, \lambda_2]$ her.\\

Die erste Ingredienz f"ur das Gelingen der Approximation ist eine rationale Funktion $\r\colon\C\to\C$ mit $\r(\R) \subseteq \R$, welche n"aherungsweise der Indikatorfunktion von $I$ entspricht.
...
\]
gilt. Um dieses Integral mit einer Gau"s'schen Quadraturformel ann"ahern zu k"onnen, ben"otigen wir eine Parametrisierung von $\mathcal{C}$. Hierf"ur ziehen wir \cite{ptep} zurate und "ubernehmen s"amtliche Rechenschritte.
Demzufolge w"ahlen wir die Funktion
\[
\gamma\colon[-1,3]\to\C\text{, }
t\mapsto c+re^{\iota \frac{\pi}{2}(1+t)}
\]
als Parametrisierung der Sph"are $\mathcal{C}$. Die
