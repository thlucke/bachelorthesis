...
Anstatt jede Spalte von $Y_{(k)}$ separat zu normalisieren, wird eine QR-Zerlegung bem"uht.

\begin{algorithm}\label{alg:chap3rriter}
\caption{Verallgemeinerte Potenzmethode (Vgl. ~\cite[115]{saad})}\label{euclid}
\begin{algorithmic}[1]
\State W"ahle linear unabh"angige Vektoren $\{x_i\}_{i=1:m}$ und setze $X_{(0)}\gets[x_i]_{i=1:m}$ und $k\gets 1$
\State \textbf{repeat}
\State \ \ \ \ Setze $X_{(k)} \gets AX_{(k-1)}$ und berechne QR-Zerlegung $X_{(k)} = QR$.
\State \ \ \ \ Setze $X_{(k)} \gets Q$ und $k\gets k+1$.
